...
import { FileText, Download, Reply, Forward, Star, Pin, Info, Trash, MoreVertical, Play, UserPlus, UserCheck, Share2 } from 'lucide-react';
  ...
  return (
    <div className="my-2 p-4 rounded-2xl bg-card border border-white/25 shadow-neo-out-sm w-full max-w-[280px] flex flex-col items-center gap-4 transition-all hover:shadow-neo-out-md hover:border-secondary/30">
      ...
      <div className="w-full pt-3 border-t border-border/40">
        {isSelf ? (
          <div className="text-xs text-text-muted text-center py-2 font-medium">
            You
          </div>
        ) : isSaved ? (
...
const MessageBubbleComponent: React.FC<MessageBubbleProps> = ({ 
  ...
  const isExpiredMedia = ['IMAGE', 'VIDEO', 'AUDIO', 'DOCUMENT'].includes(message.type) && !message.media;
  ...
  if (message.isDeleted) {
    return (
      <div id={`msg-${message.id}`} className={cn("flex w-full mb-1.5 relative px-8", isOwnMessage ? "justify-end" : "justify-start")}>
        <div className={cn("flex flex-col max-w-[75%] md:max-w-[70%] relative group/bubble", isOwnMessage ? "items-end" : "items-start")}>
  ...
  const colors = ['text-teal-500', 'text-blue-500', 'text-indigo-500', 'text-cyan-500', 'text-pink-500', 'text-orange-500'];
  ...
  return (
    <div id={`msg-${message.id}`} className={cn("flex w-full mb-1.5 relative px-8", isOwnMessage ? "justify-end" : "justify-start", (showMenu || isAddModalOpen) ? "z-40" : "z-10")}>
        ...
        <div className="flex items-center gap-2 w-full">
          ...
          <div
            className={cn(
              ...
              message.isPinned && "border-2 border-amber/50 dark:border-amber/40 shadow-[0_0_12px_rgba(245,158,11,0.25)]",
              (!isImage && !isVideo && message.type !== 'GIF') && "px-3.5 py-2" 
            )}
          >
            {message.forwarded && (
              <p className={cn("text-[10px] italic opacity-60 flex items-center gap-1 mb-1 px-1", (isImage || isVideo || message.type === 'GIF') && "px-3 pt-2")}>
                ↪ Forwarded
              </p>
            )}
            ...
            {message.replyTo && (
              ...
              >
                <p className="font-semibold opacity-90 mb-0.5 text-[11px]">
                  {message.replyTo.senderId === currentUser?.id ? 'You' : getContactName(message.replyTo.senderId, sender)}
                </p>
            ...
            {!isOwnMessage && showSenderName && sender && (
              <p className={cn("text-xs font-semibold mb-1 cursor-pointer hover:underline", nameColor, (isImage || isVideo || message.type === 'GIF') && "px-3 pt-2")}>
                {getContactName(sender.id || (sender as any)?._id, sender)}
              </p>
            )}

            {isImage && message.media && (
              <div className="p-1 cursor-pointer hover:opacity-95 transition-opacity" onClick={() => onMediaClick?.(message)}>
                 <SecureMedia type="img" src={message.media.mediaId || message.media.url} alt="Media" className="w-full max-w-[300px] h-auto rounded-xl object-cover" />
              </div>
            )}
            ...
            {isVideo && message.media && (
              ...
              >
                 <SecureMedia type="video" src={message.media.mediaId || message.media.url} className="w-full max-w-[300px] h-auto rounded-xl object-cover" controls={false} />
            ...
            {isDocument && message.media && (
              <div 
                onClick={() => onMediaClick?.(message)}
                className={cn("flex items-center gap-3 p-3 mb-1 rounded-xl cursor-pointer hover:bg-black/5 dark:hover:bg-white/5", isOwnMessage ? "bg-white/10" : "bg-surface-hover")}
              >
                <div className={cn("p-2 rounded-lg", isOwnMessage ? "bg-white/20" : "bg-primary-500/10")}><FileText className="w-6 h-6" /></div>
                <div className="flex-1 overflow-hidden text-left">
                  <p className="text-sm font-medium truncate">{message.media?.name}</p>
                  <p className="text-xs opacity-70">{((message.media?.size || 0) / 1024 / 1024).toFixed(2)} MB • {message.media?.mimeType.split('/')[1]?.toUpperCase()}</p>
                </div>
                <button 
                  onClick={(e) => handleDownload(e, message.media?.mediaId || message.media?.url || '')}
                  className={cn("p-2 rounded-full transition-colors", isOwnMessage ? "hover:bg-white/20" : "hover:bg-surface-hover")}
                >
            ...
            {isAudio && message.media && (
              <div className="cursor-pointer" onClick={() => onMediaClick?.(message)}>
                <VoiceNotePlayer url={message.media?.mediaId || message.media?.url || ''} isOwnMessage={isOwnMessage} />
              </div>
            )}

            {isExpiredMedia && (
              <div className={cn("flex items-center gap-3 p-3 mb-1 rounded-xl border border-dashed select-none bg-surface/50 border-border/60 text-text-muted/80 w-full max-w-[280px]")}>
            ...
            {message.type === 'GIF' && message.gifId && (
              <div className="p-1 w-[200px] h-[150px] rounded-xl overflow-hidden">
                <GifImage
                  src={GIF_LIBRARY.find((g) => g.id === message.gifId)?.path || `/gifs/${message.gifId}.gif`}
            ...
            {message.content && message.type !== 'CONTACT' && message.type !== 'GIF' && (
              <p className={cn("text-[15px] leading-relaxed break-words", (isImage || isVideo) && "px-3 pb-2 pt-1")}>{message.content}</p>
            )}

            <div className={cn(
              "text-[10px] mt-0.5 text-right opacity-60 flex justify-end items-center gap-1",
              isOwnMessage ? "text-white/80" : "text-text-muted",
              (isImage || isVideo || message.type === 'GIF') && !message.content && "absolute bottom-2 right-3 px-2 py-0.5 bg-black/40 rounded-full backdrop-blur-sm !text-white !mt-0",
              (isImage || isVideo || message.type === 'GIF') && message.content && "px-3 pb-2"
            )}>
              {message.isPinned && <Pin className="w-3 h-3 text-primary-500 rotate-45 transform shrink-0" />}
              ...
              {isOwnMessage && (
                 <span className="ml-0.5 tracking-tighter font-bold flex items-center">
                   {(message.status as string) === 'PENDING' && <span className="text-[9px] animate-pulse">🕒</span>}
                   {(message.status as string) === 'FAILED' && <span className="text-red-500 text-xs" title="Failed to send">⚠️</span>}
                   {message.status === 'SENT' && '✓'}
        ...
        {reactionCounts.length > 0 && (
          <div className={cn(
            ...
            {reactionCounts.map(([emoji, count]) => (
              <button
                key={emoji}
                onClick={() => socket?.emit('react_message', { messageId: message.id, chatId: message.chatId, emoji })}
      ...
      {showMenu && menuCoords && createPortal(
        <div 
          style={{ 
            position: 'fixed', 
        ...
        >
           ...
           <div className="flex gap-1 border-b border-black/5 dark:border-white/5 pb-1.5 px-1 py-1 justify-between">
             {['👍', '❤️', '😂', '😮', '😢', '🙏'].map(emoji => (
               <button
                 key={emoji}
                 onClick={() => {
                   socket?.emit('react_message', { messageId: message.id, chatId: message.chatId, emoji });
                   setShowMenu(false);
                   setMenuCoords(null);
                 }}
           ...
           {onReply && (
             <button
               onClick={() => { onReply(message); setShowMenu(false); setMenuCoords(null); }}
           ...
           {onForward && !isExpiredMedia && (
             <button
               onClick={() => { onForward(message); setShowMenu(false); setMenuCoords(null); }}
               className="flex items-center gap-2 px-2.5 py-1.5 text-xs text-text-primary hover:bg-surface-hover rounded-lg text-left"
             >
           ...
           <button
             onClick={() => {
               socket?.emit('star_message', { messageId: message.id, chatId: message.chatId, isStarred: !isStarred });
               setShowMenu(false);
               setMenuCoords(null);
             }}
             className="flex items-center gap-2 px-2.5 py-1.5 text-xs text-text-primary hover:bg-surface-hover rounded-lg text-left"
           ...
           <button
             onClick={() => {
               socket?.emit('pin_message', { messageId: message.id, chatId: message.chatId, isPinned: !message.isPinned });
               setShowMenu(false);
               setMenuCoords(null);
             }}
             className="flex items-center gap-2 px-2.5 py-1.5 text-xs text-text-primary hover:bg-surface-hover rounded-lg text-left"
      ...
      {showInfo && isOwnMessage && (
        <Dialog isOpen={showInfo} onClose={() => setShowInfo(false)} title="Message Info" className="max-w-sm">
...
export const MessageBubble = memo(MessageBubbleComponent, (prevProps, nextProps) => {
  return prevProps.isOwnMessage === nextProps.isOwnMessage &&
